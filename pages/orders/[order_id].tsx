	...
	return (
		<Card className={classes.root} variant="outlined">
			<CardContent>
				...
				<hr />
				{items.map(item => {
					return <Typography key={item._id}>x{item.quantity} {item.name} por R$ {(parseInt(item.amount) / 100).toFixed(2)}</Typography>
				})}
				<hr />
				...
				{payments.length > 0 && payments.map(payment => {
					return <Typography key={payment._id}>Valor ({payment.method.toLowerCase()}): R$ {(payment.amount / 100).toFixed(2)}</Typography>
				})}
			...
			<CardActions>
				<Button size="small" onClick={() => router.back()}>Voltar</Button>
			</CardActions>
		</Card>
	);
}
